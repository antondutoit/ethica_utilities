...
# [  ] find complete list of 3-character activity extensions in Avicenna docs 
...
```{r Create: read in code from source file}
...
```{r create:  arrange by answer date-time and add a column with ordinal no of observation}
for (df_name in df_names)  {
  df_list[[df_name]] %<>% arrange(record_time)
  activity_no <- str_extract(df_name, pattern = pattern6) %>% str_remove_all("df") %>% str_remove_all("_")
  new.var <- paste("obs", activity_no, sep = "." )
  assign(new.var, 1:nrow(df_list[[df_name]]))
  df_list[[df_name]] %<>% mutate("obs.{activity_no}" := eval(sym(new.var)), .before = uuid)
  }
...
for (df_name in df_names)  {
  df_list[[df_name]] %<>% select(all_of(sortvec), starts_with("obs."), everything())
