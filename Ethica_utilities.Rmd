...
# NB backslashes must be changed into forward slashes   
# [ ] make or find a wee function to do this   
...
study_number <- str_extract(file_names[1], pattern = 
                                 "activity_response_" %R% DGT %R% DGT %R% DGT %R% DGT %R% optional(DGT))
...
str_detect(x5, pattern1.2)  #  


number_extract_single2 <- function(x) {
  as.numeric(str_replace(x, pattern = pattern1.2, replacement = REF1))
  }


# detect answer columns and rewrite content 
for (df_name in df_names)  {
...
logV <- str_detect(column_names, pattern = pattern3)
...
```{r test}
str_extract(file_name, pattern = 
                                 DGT %R% DGT %R% DGT %R% DGT %R% optional(DGT) %R%
                                 '_' %R% DGT %R% DGT %R% DGT %R% DGT %R% optional(DGT) %R%
                                 '_' %R% DGT %R% optional(DGT) %R% optional(DGT)) 
```
