...
# [  ] write function(s) to format answer columns as the correct type (numeric, character, etc)

# [  ] evaluate if I need the MAQ processing code -- may have been superceded by what Avicenna now provides 
# [  ] consider a study metadata df (or list) as another output of the suggested workflow. 
#       This could include a data dictionary and labels
# [ip] add naniar fun to create col with no. NA per line 
...
```{r Create: constants and patterns for regex - will go into source file}
...
# used to detect and remove unwanted columns
...
# used to extract activity no. from df_name
...
```{r Create: functions for use in rest of script - will go into source file}
...
if (sum(maqV)>0) {
  ...
  maq_option_no  %<>%  str_replace_all(capture(repeated(DGT,1,2)), "_opt" %R% REF1)
...
```{r create:  Add a column with number of missing values in that row}
for (df_name in df_names)  {
  df_list[[df_name]] %<>% naniar::add_n_miss()  
 }

```

```{r Create: change some var types}
for (df_name in df_names)  {
  
df_list[[df_name]] %<>% select(all_of(sortvec), everything())
...
```{r checks of length for rename answer columns chunk, eval=TRUE}
length(column_names)
length(logV)
...
# first approximation
combined_df <- bind_rows(df_list, .id = "obs_id")

combined_df %<>% select(all_of(sortvec), everything()) 
